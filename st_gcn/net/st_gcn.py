...
class Model(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_point,
                 num_person=1,
                 use_data_bn=False, #true
                 backbone_config=None,
                 graph=None,  #st_gcn.graph.Hanchoom
                 graph_args=dict(), #labeling_mode: 'spatial'
                 mask_learning=False, #true
                 use_local_bn=False,
                 multiscale=False,
        ...
        if graph is None:
            ...
            #print("ggg",graph)
            Graph = import_class(graph)  #st_gcn.graph.hanchoom.Graph
            #print("bbbbbb", Graph)
            #print("aaaaaaaaaaaaaaa",graph_args) 
            self.graph = Graph(**graph_args) #spatial
            #print(self.graph)
            self.A = torch.from_numpy(self.graph.A).float().cuda(0)  #ad matrix

        self.num_class = num_class #8
        self.use_data_bn = use_data_bn #true
        self.multiscale = multiscale #false

        # Different bodies share batchNorma parameters or not
        self.M_dim_bn = True

        if self.M_dim_bn:
            self.data_bn = nn.BatchNorm1d(channel * num_point * num_person) #3*18*1
        else:
            self.data_bn = nn.BatchNorm1d(channel * num_point)

        kwargs = dict(
            A=self.A, #ad
            mask_learning=mask_learning, #true
            use_local_bn=use_local_bn, #F
            dropout=dropout, #...
            kernel_size=temporal_kernel_size) #9
        ...
        self.backbone = nn.ModuleList([unit(in_c, out_c, stride=stride, **kwargs)for in_c, out_c, stride in backbone_config])
        ...
        # head
        self.gcn0 = unit_gcn( #(in64,out64,stride1) , ...
            channel,
            backbone_in_c,
            self.A,
